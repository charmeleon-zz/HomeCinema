    ...
    function customersRegCtrl($scope, $location, $rootScope, apiService) {
        ...
        function Register() {
            apiService.post('/api/customers/register', $scope.newCustomer,
                registerCustomerSucceeded, registerCustomerFailed);
        }
        function registerCustomerSucceeded(response) {
            ...
            $scope.submission.successMessages.push($scope.newCustomer.FirstName + ' ' +
                $scope.newCustomer.LastName + ' has been successfully registered');
